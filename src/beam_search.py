...
def beam_search(decoder, input_features, vocabulary, beam_size):
  ...
  prev_topk_words = torch.LongTensor([[vocabulary.word2id['<start>']]] * beam_size).cuda()
  prev_topk_wordembeds = decoder.module.word_embed(Variable(prev_topk_words)).squeeze(1)
  ...
  hidden_dim = decoder.module.hidden_size
  ...
  for t in range(max_cap_len):
    if(t!=0):
      prev_topk_wordembeds = decoder.module.word_embed(Variable(prev_topk_words)).squeeze(1)
      alpha, context = decoder.module.predict_attention(input_features.cuda(), h_curr)

    decoder_input = torch.cat([context, prev_topk_wordembeds], dim=1)
    h_curr, c_curr = decoder.module.lstm_cell(decoder_input, (h_curr, c_curr))
    scores = decoder.module.deep_output(h_curr)
    ...
    alpha_squences = alpha_squences[incomp_captions]
...
def findk_wordseq_alphas(topk_words, squences, alpha_squences, vocab_size, alpha):
  prev_words = topk_words/vocab_size
  curr_words = topk_words%vocab_size
  squences = torch.cat([squences[prev_words], curr_words.unsqueeze(1).long()], dim=1)
...
def find_sequences(topk_words, vocabulary):
  curr_words = topk_words%(vocabulary.count)
  incomp_captions = [ind for ind, curr_word in enumerate(curr_words) if curr_word != vocabulary.word2id["<end>"]]
